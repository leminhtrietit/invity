...
export async function POST(request:Request,{params}:{params:Promise<{publicCode:string}>}){
  ...
  const {publicCode}=await params;const supabase=createPublicSupabaseClient();const {data,error}=await supabase.rpc("resolve_public_gift_recipient",{p_public_code:publicCode,p_account_id:parsed.data.recipientId});
  ...
  await supabase.rpc("track_product_event",{p_event_name:"qr_generated",p_anonymous_key:requestFingerprint(request),p_event_id:null,p_template_id:null});
