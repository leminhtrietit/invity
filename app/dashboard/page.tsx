    ...
    <>
      ...
      <main className="shell py-14">
        ...
        {events?.length ? <section className="dashboard-events"><div className="dashboard-section-heading"><h2 className="display">Thiệp của bạn</h2><Link href="/templates">Khám phá mẫu khác →</Link></div><div className="dashboard-event-grid">{events.map((event) => {
          ...
          return <article className="dashboard-event-card" key={event.id}><div className="dashboard-event-art"><span>{getTemplate(event.template_id)?.motif ?? "✦"}</span></div><div><p>{getTemplate(event.template_id)?.name ?? event.template_id} · {lifecycleLabel}</p><h3 className="display">{title}</h3><small>Cập nhật {new Intl.DateTimeFormat("vi-VN", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Ho_Chi_Minh" }).format(new Date(event.updated_at))}</small><div className="dashboard-event-actions"><Link className="button button-secondary" href={`/events/${event.id}/edit`}>Tiếp tục sửa</Link>{event.lifecycle === "published" && <Link href={`/e/${event.public_code}`} target="_blank">Link khách ↗</Link>}{event.lifecycle === "draft" && <DeleteDraftButton eventId={event.id} />}</div></div></article>;
