...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ templateId?: string }> }) {
  ...
  const { data: events } = await supabase.from("events").select("id,template_id,lifecycle,public_code,updated_at,event_drafts(revision,content)").neq("lifecycle", "deleted").order("updated_at", { ascending: false });
  ...
  return (
    <>
      ...
      <main className="shell py-14">
        ...
        <section className="dashboard-summary"><div><p className="text-sm font-bold text-[var(--accent)]">GÓI FREE</p><strong>{events?.filter((event) => event.lifecycle === "draft").length ?? 0}<span>/3 bản nháp</span></strong></div><CreateEventButton templateId={selectedTemplateId} /></section>
        {events?.length ? <section className="dashboard-events"><div className="dashboard-section-heading"><h2 className="display">Thiệp của bạn</h2><Link href="/templates">Khám phá mẫu khác →</Link></div><div className="dashboard-event-grid">{events.map((event) => {
          ...
          const lifecycleLabel = event.lifecycle === "published" ? "Đã xuất bản" : event.lifecycle === "hidden" ? "Đang tạm ẩn" : "Bản nháp";
          return <article className="dashboard-event-card" key={event.id}><div className="dashboard-event-art"><span>{getTemplate(event.template_id)?.motif ?? "✦"}</span></div><div><p>{getTemplate(event.template_id)?.name ?? event.template_id} · {lifecycleLabel}</p><h3 className="display">{title}</h3><small>Cập nhật {new Intl.DateTimeFormat("vi-VN", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Ho_Chi_Minh" }).format(new Date(event.updated_at))}</small><div className="dashboard-event-actions"><Link className="button button-secondary" href={`/events/${event.id}/edit`}>Tiếp tục sửa</Link><Link href={`/events/${event.id}/gifts`}>Quà mừng</Link>{event.lifecycle === "published" && <><Link href={`/events/${event.id}/guests`}>Khách mời</Link><Link href={`/e/${event.public_code}`} target="_blank">Link khách ↗</Link></>}{event.lifecycle === "draft" && <DeleteDraftButton eventId={event.id} />}</div></div></article>;
