...
| INV-103 | Hoàn tất production | Email/password và OIDC LeMinhTriet qua Supabase Auth, Authorization Code + PKCE, callback/session SSR, logout và kiểm tra `returnTo` |
| INV-104 | Hoàn tất production | `app_users`, `auth_bindings`, `external_identities`, RLS và RPC bootstrap có khóa chống race; email bind bằng provider subject, OIDC bind bằng `(iss, sub)` |
...
- Chỉ danh tính email đã xác minh, Google legacy hoặc `custom:leminhtriet` được RPC bootstrap chấp nhận; subject lấy từ `auth.identities`.
...
Biến môi trường Vercel đã cấu hình bằng publishable key; không đưa service-role key lên browser. Supabase Auth `site_url` và allowlist callback được giới hạn theo môi trường. Email/password chỉ đăng nhập tài khoản có sẵn; giao diện không tự mở đăng ký.
