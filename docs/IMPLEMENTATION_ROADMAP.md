...
> Cập nhật 24/09/2026: Supabase `invity` đã Resume từ `INACTIVE` về `ACTIVE_HEALTHY`. Workflow retention hằng ngày đã chạy thủ công thành công trên production; smoke test được mở rộng để kiểm tra Auth và database. Theo quyết định PO, giữ Supabase Free và chưa chọn nơi lưu backup riêng. UAT thiết bị thật cùng backup/restore drill vẫn chưa có bằng chứng nghiệm thu, nên chưa mở gate G11.
