...
alter table public.auth_bindings
  ...
  check (provider in ('google', 'leminhtriet_oidc'));
...
begin
  ...
  if auth_record.id is null or raw_provider not in ('google', 'custom:leminhtriet') then
  ...
  subject := coalesce(identity_record.identity_data ->> 'sub', identity_record.provider_id);
  ...
  if raw_provider = 'custom:leminhtriet' then
    ...
    binding_subject := identity_issuer || '|' || subject;
  else
    binding_provider := 'google';
  ...
  if raw_provider = 'custom:leminhtriet' then
    ...
    values (stable_app_user_id, identity_issuer, subject)
